...
def gui(infile, chapter, option):
    ...
    def process_word(*args):
        return w.interactive(question, answer, correct, suggestion)

    ttk.Label(mainframe, text="Please translate: ").grid(
        column=1, row=1, sticky=E)
    ttk.Label(mainframe, textvariable=question).grid(
    ...
    ttk.Button(mainframe, text="Check", command=process_word).grid(
        column=3, row=4, sticky=W)

    ttk.Label(mainframe, text="Correct answer: ").grid(
        column=1, row=3, sticky=E)
    ttk.Label(mainframe, textvariable=correct).grid(
        column=2, row=3, sticky=(W, E))

    ttk.Label(mainframe, text="Your suggestion: ").grid(
        column=1, row=4, sticky=E)
    ttk.Label(mainframe, textvariable=suggestion).grid(
